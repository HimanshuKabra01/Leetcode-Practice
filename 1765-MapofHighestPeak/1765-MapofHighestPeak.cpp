// Last updated: 7/22/2026, 8:50:33 PM
1class Solution {
2public:
3    vector<vector<int>> highestPeak(vector<vector<int>>& mat) {
4        int m = mat.size();
5        int n = mat[0].size();
6
...
24            q.pop();
25
26            if(i - 1 >= 0 && !vis[i-1][j] && mat[i-1][j] == 0) {
27                q.push({i-1, j});
28                ans[i-1][j] = ans[i][j] + 1;
29                vis[i-1][j] = true;
30            }
31            
32            if(j + 1 < n && !vis[i][j+1] && mat[i][j+1] == 0) {
33                q.push({i, j+1});
34                ans[i][j+1] = ans[i][j] + 1;
35                vis[i][j+1] = true;
36            }
37            
38            if(i + 1 < m && !vis[i+1][j] && mat[i+1][j] == 0) {
39                q.push({i+1, j});
40                ans[i+1][j] = ans[i][j] + 1;
41                vis[i+1][j] = true;
42            }
43            
44            if(j - 1 >= 0 && !vis[i][j-1] && mat[i][j-1] == 0) {
45                q.push({i, j-1});
